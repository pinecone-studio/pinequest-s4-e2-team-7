...
/** How often we fold the live zone into the ML arch coverage (ms). */
const TICK_MS = 250

/**
 * Drives the per-tooth ML arch coverage (`brushMl`) from the recognizer's live
 * zone label. The IMU only resolves quadrant × surface, so the tooth index is
 * swept synthetically within the active quadrant to spread coverage across the
 * arch — same spirit as `synthesizeSensorFrame`.
 */
