...
import { HandRaisedIcon, PhoneIcon, CheckCircleIcon } from '@heroicons/react/24/outline'
        ...
        {profile ? (
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-sm text-text-base">
              ...
              <span>Сайн дураар бүртгэлтэй: <b>{profile.displayName}</b>{profile.org ? ` · ${profile.org}` : ''}</span>
            </div>
            <button
          ...
          <form
            onSubmit={(e) => { e.preventDefault(); if (name.trim()) upsert.mutate({ displayName: name, org: org || undefined, area: area || undefined }) }}
          ...
          >
            ...
            <button className="btn self-start rounded-lg bg-primary px-4 py-2 text-sm font-medium text-text-on-primary hover:bg-primary-hover">Бүртгүүлэх</button>
          </form>
        )}
      </Card>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div>
            ...
            {connected.length === 0 ? (
              <EmptyState Icon={CheckCircleIcon} title="Холбогдсон хүсэлт алга" />
            ) : (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
