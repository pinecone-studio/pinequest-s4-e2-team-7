...
// One content version. Drafts get an in-place Publish (the dentist-approval
// gate): button cycles idle → publishing → the status pill flips to "Нийтлэгдсэн".
  ...
  return (
    <div className="flex items-center gap-3 border-t border-border-muted px-5 py-3.5 first:border-t-0">
      ...
      {published ? (
        ...
        >
          {publish.isPending ? 'Нийтэлж байна…' : <><PaperAirplaneIcon className="size-3.5" /> Нийтлэх</>}
        </button>
      )}
    </div>
  )
