...
const LEVEL_LABEL: Record<string, string> = { green: 'Нормаль', yellow: 'Хянах шаардлагатай', red: 'Яаралтай' }
...
const fmtDate = (iso: string) => new Date(iso).toLocaleDateString('mn-MN', { year: 'numeric', month: 'long', day: 'numeric' })

const activeSymptoms = (q: ScreeningDetail['questionnaire']): string[] =>
  q ? Object.entries(SYMPTOM_LABEL).filter(([k]) => (q as Record<string, boolean | null>)[k]).map(([, v]) => v) : []
...
const buildHtml = (d: ScreeningDetail): string => {
  ...
  const levelColor = d.triageLevel === 'green' ? '#3B8C5E' : d.triageLevel === 'red' ? '#A84545' : '#A8580A'
  const symptoms = activeSymptoms(d.questionnaire)
  const findingsRows = d.findings.length
    ? d.findings.map((f) => `<tr><td>${f.fdi ?? '—'}</td><td>${f.className}</td><td>${Math.round(f.confidence * 100)}%</td></tr>`).join('')
  ...
  <style>
    ...
    <div class="grid">
      ...
      <div><div class="label">Анги</div>${d.classId}</div>
    </div>
...
export default function ScreeningDetailScreen() {
  ...
  const { id, childName, level } = useLocalSearchParams<{ id: string; childName?: string; level?: string }>()
  const [detail, setDetail] = useState<ScreeningDetail | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [sharing, setSharing] = useState(false)

  useEffect(() => {
    getScreening(id).then(setDetail).catch((e: unknown) => setError(toMongolian(e)))
  }, [id])
  ...
  const levelColor = shownLevel === 'green' ? colors.triageGreenText : shownLevel === 'red' ? colors.triageRedText : colors.triageYellowText
  const badgeBg = shownLevel === 'green' ? colors.badgeGreen : shownLevel === 'red' ? colors.badgeRed : colors.badgeYellow

  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={[s.safe, { backgroundColor: colors.bg }]}>
      <View style={s.head}>
        <ScreenHeader title="Скрининг тайлан" subtitle={shownName} />
      </View>
      ...
      {!detail && !error ? (
        ...
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.page, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text style={[s.docTitle, { color: colors.textBase }]}>Шүдний скрининг — тайлан</Text>
            <Text style={[s.docSub, { color: colors.textMuted }]}>Урьдчилсан чиглүүлэг (онош биш). Шүдний эмч эцэслэн батална.</Text>
            ...
            <View style={s.grid}>
              ...
              <Field label="Төрсөн он" value={detail.childBirthYear ? String(detail.childBirthYear) : '—'} colors={colors} />
              ...
              <Field label="Анги" value={detail.classId} colors={colors} />
            </View>
            ...
            {detail.findings.length ? detail.findings.map((f) => (
              <View key={f.id} style={[s.tr, { borderColor: colors.border }]}>
                <Text style={[s.td, s.col1, { color: colors.textBase }]}>{f.fdi ?? '—'}</Text>
                <Text style={[s.td, s.col2, { color: colors.textBase }]}>{f.className}</Text>
                <Text style={[s.td, s.col3, { color: levelColor }]}>{Math.round(f.confidence * 100)}%</Text>
              </View>
            )) : (
              <Text style={[s.muted, { color: colors.textMuted, paddingVertical: 8 }]}>Илрэл бүртгэгдээгүй</Text>
            )}

            <Text style={[s.section, { color: colors.textBase }]}>Асуумжийн шинж тэмдэг</Text>
            {activeSymptoms(detail.questionnaire).length ? activeSymptoms(detail.questionnaire).map((sym) => (
              ...
              <Text style={[s.muted, { color: colors.textMuted, paddingVertical: 4 }]}>Шинж тэмдэг бүртгэгдээгүй</Text>
            )}

            {detail.review ? (
              <View style={[s.reviewBox, { borderColor: colors.border }]}>
                <Text style={[s.label, { color: colors.textMuted }]}>ШҮДНИЙ ЭМЧИЙН БАТАЛГААЖУУЛАЛТ</Text>
                <Text style={[s.reviewLevel, { color: colors.textBase }]}>{LEVEL_LABEL[detail.review.confirmedLevel] ?? detail.review.confirmedLevel}</Text>
                {detail.review.note ? <Text style={[s.reviewNote, { color: colors.textBase }]}>{detail.review.note}</Text> : null}
              </View>
            ) : null}

            <Text style={[s.foot, { color: colors.textDisabled, borderColor: colors.border }]}>
              Энэ тайлан нь утсан камераар хийсэн урьдчилсан скринингийн дүн бөгөөд оношилгоо биш.
              Ногоон = эдгээр зурагт аюулын шинж ажиглагдаагүй. Эцсийн дүгнэлтийг шүдний эмч өгнө.
            </Text>
          </View>
        </ScrollView>
      ) : null}
      ...
      {detail ? (
        <TouchableOpacity style={[s.shareBtn, { backgroundColor: colors.primary }]} onPress={onShare} disabled={sharing} activeOpacity={0.85}>
          {sharing ? <ActivityIndicator color={colors.primaryText} /> : (
            <>
              <Ionicons name="share-outline" size={18} color={colors.primaryText} />
              <Text style={[s.shareText, { color: colors.primaryText }]}>PDF болгож хадгалах / хуваалцах</Text>
            </>
          )}
        </TouchableOpacity>
      ) : null}
...
const Field = ({ label, value, colors }: { label: string; value: string; colors: { textMuted: string; textBase: string } }) => (
  <View style={s.field}>
...
const s = StyleSheet.create({
  ...
  pill: { alignSelf: 'flex-start', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6, marginTop: 16 },
  ...
  foot: { fontSize: 11, fontFamily: 'Inter_400Regular', lineHeight: 17, marginTop: 24, borderTopWidth: 1, paddingTop: 12 },
  muted: { fontSize: 14, fontFamily: 'Inter_400Regular' },
  shareBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, margin: 16, marginTop: 4, borderRadius: 14, padding: 16 },
